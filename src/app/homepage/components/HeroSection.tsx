...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden bg-background chekku-pattern pt-24 pb-12">
      {/* Decorative background circles */}
      <div className="absolute top-20 right-0 w-[600px] h-[600px] rounded-full bg-accent/5 -translate-y-1/4 translate-x-1/4 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-primary/5 translate-y-1/3 -translate-x-1/3 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 xl:col-span-7 flex flex-col gap-8">
            ...
            <h1
              ref={headlineRef}
              className="font-serif text-[clamp(2.8rem,7vw,6rem)] font-light leading-[0.92] tracking-tight text-foreground">
            ...
            <div className="flex flex-col sm:flex-row gap-4 animate-fade-slide-up delay-400">
              ...
              <Link
                href="/products"
                className="flex items-center justify-center gap-2 text-base font-semibold text-primary border-2 border-primary/30 rounded-full px-6 py-3.5 hover:bg-primary/5 hover:border-primary transition-all">
            ...
            {/* Stats row */}
            <div className="flex flex-wrap gap-6 pt-2 border-t border-border animate-fade-slide-up delay-500">
              {[
              ...
              <div key={s?.label} className="flex flex-col">
                  <span className="font-serif text-2xl font-medium text-primary">{s?.num}</span>
                  <span className="text-xs font-semibold tracking-wider uppercase text-muted-foreground">{s?.label}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-6 xl:col-span-5 relative">
            <div className="relative animate-float">
              {/* Main image card */}
              <div className="relative rounded-3xl overflow-hidden border border-border shadow-2xl bg-muted aspect-[4/5] max-w-sm mx-auto lg:mx-0 lg:ml-auto">
                <AppImage
                ...
                <div className="absolute bottom-6 left-6 right-6 glass-card rounded-2xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-foreground">Wooden Chekku Extracted</p>
                      <p className="text-xs text-muted-foreground">Ancient cold press method</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute -top-4 -right-4 lg:-right-8 glass-card rounded-2xl p-4 shadow-xl border border-border hidden sm:flex flex-col gap-1 min-w-[140px]">
                <span className="text-2xl">🌿</span>
                <p className="text-sm font-bold text-primary">Farm to Bottle</p>
                <p className="text-xs text-muted-foreground">Direct sourcing</p>
              </div>

              {/* Floating product badge */}
              <div className="absolute -bottom-4 -left-4 lg:-left-8 glass-card rounded-2xl px-4 py-3 shadow-xl border border-border hidden sm:block">
                <p className="text-xs font-semibold tracking-wider uppercase text-muted-foreground">Starting from</p>
                <p className="font-serif text-2xl font-medium text-accent">₹180</p>
                <p className="text-xs text-muted-foreground">500ml bottle</p>
              </div>
            </div>
          </div>
        </div>
